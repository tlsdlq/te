  ...
  async fetch(request, env, ctx) {
    try {
      ...
      if (!imgUrl || !text) {
        ...
        const usageText = "Usage: ?img=&text=&name=";
...
function createSvg({ width, height, imageUrl, image, lines, fontSize, name }, doCache) {
  ...
  const textElements = lines.map((line, i) => `${esc(line)}`).join('');

  const svg = `.txt{font-family:${FONT_FAMILY};font-size:${fontSize}px;fill:white;font-weight:600;}.name{font-family:${FONT_FAMILY};font-size:${fontSize * 0.75}px;fill:white;font-weight:700;}${nameEl}${textElements}`;
