...
async function getImageDimensions(url, context) {
  // ... (이전과 동일)
  ...
  try {
    ...
    if (dimensions) {
      context.waitUntil(cache.put(cacheKey, new Response(JSON.stringify(dimensions), { headers: { 'Cache-Control': 'public, max-age=2592000' } })));
      return dimensions;
    }
...
export async function onRequest(context) {
  ...
  if (imageUrl) {
    try {
      ...
      // 2. 허용 목록(Whitelist) 검사
      if (!ALLOWED_IMAGE_HOSTS.includes(imageUrlObject.hostname)) {
        throw new Error('Host not allowed');
      }
    } catch (e) {
      // URL 형식이 잘못되었거나, 프로토콜이 다르거나, 허용 목록에 없는 호스트일 경우 400 오류 반환
  ...
  if (!imageUrl || !text) {
    // 사용법 안내용 이미지 URL이 허용 목록에 있는지 확인
  ...
  const { width, height } = await getImageDimensions(imageUrl, context);
...
function generateSvgResponse({ width, height, imageUrl, lines, fontSize, name }) {
  ...
  if (name) {
    ...
    nameElements = `
      <polygon points="${p1} ${p2} ${p3} ${p4}" fill="rgba(255,255,255,0.25)"/>
      <text x="${nameBoxPadding}" y="${boxY + nameBoxHeight / 2}" class="name-caption" dominant-baseline="central">
        ${escapeXml(name)}
      </text>
    `;
  }
...
<svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" xmlns="http://www.w3.org/2000/svg">
  <defs>
    <style>
      .caption { font-family: ${FONT_FAMILY}; font-size: ${fontSize}px; fill: white; font-weight: 600; }
      .name-caption { font-family: ${FONT_FAMILY}; font-size: ${fontSize * 0.75}px; fill: white; font-weight: 700; }
    </style>
  </defs>
  <image href="${escapeXml(imageUrl)}" x="0" y="0" width="100%" height="100%" preserveAspectRatio="xMidYMid slice" />
  <rect x="0" y="${boxY}" width="100%" height="${boxHeight}" fill="rgba(0,0,0,0.6)" />
  ${nameElements}
  <text x="${PADDING}" y="${boxY + textOffsetY + fontSize}" class="caption">
