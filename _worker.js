    ...
    async fetch(request, env, ctx) {
        ...
        try {
            ...
            if (!imgUrl || !text) {
                ...
                const usageText = "Usage: ?img=<img_url>&text=<text>&name=<name>";
...
function createSvg({ width, height, imageUrl, image, lines, fontSize, name }, doCache) {
    ...
    if (name) {
        ...
        const nameY = boxY + PADDING + nameFS;
        nameEl = `<text x="${PADDING}" y="${nameY}" class="name">${esc(name)}</text>`;
    }

    const textOffsetY = name ? fontSize * 1.5 : PADDING * 0.5;
    const textElements = lines.map((line, i) => `<text x="${PADDING}" y="${boxY + textOffsetY + (i * fontSize * LINE_HEIGHT) + PADDING}" class="txt">${esc(line)}</text>`).join('');
    ...
    <defs>
        <style>
            .txt{font-family:${FONT_FAMILY};font-size:${fontSize}px;fill:white;font-weight:600;}
            .name{font-family:${FONT_FAMILY};font-size:${fontSize * 0.75}px;fill:white;font-weight:700;}
        </style>
    </defs>
    <image href="${imgHref}" width="${width}" height="${height}" />
    <rect x="0" y="${boxY}" width="${width}" height="${boxHeight}" fill="rgba(0,0,0,0.5)" />
    ${nameEl}
    ${textElements}
</svg>`;
    ...
    if (doCache) {
        ...
        responseOptions.cf = { cacheEverything: true, cacheTtl: 2592000 };
    }
...
async function getImage(url, ctx) {
    ...
    if (length && parseInt(length, 10) > MAX_SIZE) {
        throw new Error(`Image size exceeds ${MAX_SIZE / 1024 / 1024}MB`);
    }
    ...
    const image = { contentType, base64: Buffer.from(buffer).toString('base64') };
    const view = new DataView(buffer);
    let dims = { width: DEF_WIDTH, height: DEF_HEIGHT };
    ...
    try {
        if (view.getUint16(0, false) === 0xFFD8) {
            ...
            dims = { width: view.getUint32(16, false), height: view.getUint32(20, false) };
        } else if (view.getUint32(8, false) === 0x57454250) {
            const chunk = new TextDecoder().decode(view.subarray(12, 16));
            if (chunk === 'VP8 ') {
                if ((view.getUint32(19, true) & 0x07) === 0) {
                    dims = { width: view.getUint16(26, true) & 0x3FFF, height: view.getUint16(28, true) & 0x3FFF };
                }
            } else if (chunk === 'VP8L') {
                const bits = view.getUint32(21, true);
                dims = { width: (bits & 0x3FFF) + 1, height: (((bits >> 14) & 0x3FFF)) + 1 };
            } else if (chunk === 'VP8X') {
                dims = { width: view.getUint32(24, true) + 1, height: view.getUint32(28, true) + 1 };
            }
        }
    } catch (e) { /* ignore parsing errors, use default dims */ }

    const result = { image, width: dims.width, height: dims.height };
...
function TW(text, fontSize) {
    ...
    for (const char of text) {
        total += wideChar.test(char) ? fontSize : fontSize * 0.55;
    }
    return total;
}

function wrap(text, maxWidth, fontSize, widthCalc) {
    ...
    for (let i = 1; i < words.length; i++) {
        ...
        if (widthCalc(potentialLine, fontSize) < maxWidth) {
